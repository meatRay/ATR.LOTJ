...
class Cargo
{
    ...
    public static SearchOptions LoadSearchFromFile( string filename = "Search.json" )
    {
        if ( File.Exists( filename ) )
        {
            using var fl = File.OpenRead( filename );
            var got = JsonSerializer.Deserialize<SearchOptions>( fl, JsonOptions );
            if ( got is not null )
                return got;
            return new( 0.6, true );
        }
        ...
        {
            SearchOptions dict = new( 0.6, true );
            using var fl = File.OpenWrite( filename );
            JsonSerializer.Serialize( fl, dict, JsonOptions );
            return dict;
        }
    }
    public static ShipOptions LoadShipFromFile( string filename = "Ships.json" )
    {
        SearchOptions config = new( 0.6, true );
        if ( File.Exists( filename ) )
        {
            using var fl = File.OpenRead( filename );
            var got = JsonSerializer.Deserialize<ShipOptions>( fl, JsonOptions );
            if ( got is not null )
                return got;
            return new( 7000, 75, 100 );
        }
        else
        {
            ShipOptions dict = new( 7000, 75, 100 );
            using var fl = File.OpenWrite( filename );
            JsonSerializer.Serialize( fl, dict, JsonOptions );
            return dict;
        }
    }
    public static IList<NamedPlanet> LoadPlanetsFromFile( string filename = "Planets.json" )
    {
        if ( File.Exists( filename ) )
        {
            using var fl = File.OpenRead( filename );
    ...
    public static void SaveSearchToFile( SearchOptions planets, string filename = "Search.json" )
    {
        using var fl = File.Open( filename, FileMode.Create, FileAccess.Write );
        //using var fl = File.OpenWrite( filename );
        JsonSerializer.Serialize( fl, planets, JsonOptions );
    }
    public static void SaveShipToFile( ShipOptions planets, string filename = "Ships.json" )
    {
        using var fl = File.Open( filename, FileMode.Create, FileAccess.Write );
        //using var fl = File.OpenWrite( filename );
        JsonSerializer.Serialize( fl, planets, JsonOptions );
    }
    public static void SavePlanetsToFile( IList<NamedPlanet> planets, string filename = "Planets.json" )
    {
        using var fl = File.Open( filename, FileMode.Create, FileAccess.Write );
        //using var fl = File.OpenWrite( filename );
        JsonSerializer.Serialize( fl, planets, JsonOptions );
